...
| Comando | Descrição |
| :---| :--- |
|`npx tsx src/cli.ts add --description Cinema --amount 30`| Adiciona uma nova despesa com descrição e valor.|
|`npx tsx src/cli.ts list`|Lista todas as despesas cadastradas.|
|`npx tsx src/cli.ts delete < id >`| Deleta uma despesa.|
|`npx tsx src/cli.ts summary`|Mostra um resumo de todas as despesas.|
|`npx tsx src/cli.ts summary --month 1` |Monstra um resumo de todas as despesa de um mês.|
|`npx tsx src/cli.ts update < id > --description New description --value 10`| Atualiza descrição e valor de uma despesa.|
